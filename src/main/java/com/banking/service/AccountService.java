...
//				ResultSet rs = stmt.executeQuery();
//				if (rs.next()) {
//					BigDecimal totalWithdrawals = rs.getBigDecimal("total").abs(); // Convert to positive
//					report.put("totalWithdrawals", totalWithdrawals);
//				}
//			}
//
//			// ✅ Fix: Ensure we subtract withdrawals to calculate total change correctly
//			BigDecimal totalDeposits = (BigDecimal) report.get("totalDeposits");
//			BigDecimal totalWithdrawals = (BigDecimal) report.get("totalWithdrawals");
//			BigDecimal totalChange = totalDeposits.subtract(totalWithdrawals);
//			report.put("totalChange", totalChange); // Store the correct value
//
//		} catch (SQLException e) {
...
//                    "SELECT SUM(amount) as total " +
					"SELECT COALESCE(SUM(amount), 0) as total " + // Added COALESCE to handle NULL values
							"FROM transactions " +
							"WHERE DATE(transaction_date) = CURRENT_DATE " +
							"AND amount > 0";
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
...
//                    "SELECT SUM(amount) as total " +
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				ResultSet rs = stmt.executeQuery();
				if (rs.next()) {
					BigDecimal totalWithdrawals = rs.getBigDecimal("total").abs(); // Convert negative to positive
					report.put("totalWithdrawals", totalWithdrawals); // Store positive withdrawal amount
                    //BigDecimal total = rs.getBigDecimal("total");
